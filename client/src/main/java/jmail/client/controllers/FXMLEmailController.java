...
public class FXMLEmailController extends AnchorPane {
  ...
  public void buttonReply(ActionEvent e) {
    ...
    DataModel.getInstance().getCurrentEmail().ifPresentOrElse(email -> {
      ...
      var newEmail = new Email(UUID.randomUUID().toString(), subject, body,
          DataModel.getInstance().getCurrentUser().getEmail(), List.of(email.getSender()),
  ...
  public void buttonReplyAll(ActionEvent actionEvent) {
    ...
    DataModel.getInstance().getCurrentEmail().ifPresentOrElse(email -> {
      ...
      var newEmail = new Email(UUID.randomUUID().toString(), subject, body, userEmail, recipients,
          Calendar.getInstance().getTime(), false);
  ...
  public void buttonFwd(ActionEvent e) {
    DataModel.getInstance().getCurrentEmail().ifPresentOrElse(email -> {
      ...
      var newEmail = new Email(UUID.randomUUID().toString(), subject, body,
          DataModel.getInstance().getCurrentUser().getEmail(), new ArrayList<>(), Calendar.getInstance().getTime(),
          false);
  ...
  public void buttonTrash(ActionEvent e) {
    DataModel.getInstance().getCurrentEmail().ifPresentOrElse(email -> {
      ...
      if (DataModel.getInstance().isEditingMode()) { // clean draft
        ...
        if (hardDelete) {
          new CustomDialog(Main.primaryStage, "confirm", "Delete email", "Are you sure you want to delete this email?")
  ...
  public void buttonSend(ActionEvent e) {
    ...
    // Create a new email in order to add a new Object without reference to list
      var newEmail = new Email(UUID.randomUUID().toString(), subjectField.getText(), bodyField.getText(),
          email.getSender(), new ArrayList<>(Arrays.asList(recipientsField.getText().split(";"))),
          Calendar.getInstance().getTime(), false);

    // Check if email is valid
     var valid = ValidatorHelper.isEmailValid(newEmail);
      if (!valid.getKey()) {
        new CustomDialog(Main.primaryStage, "error", "Invalid email", valid.getValue()).showAndWait();
