...
public class FXMLLogin {
  ...
  public void initialize() {
    ...
    DataModel.getInstance().isServerStatusConnected()
        .addListener((observable, oldValue, newValue) -> Platform.runLater(() -> {
          var newFontIcon = new FontIcon("mdi2w-web-box");
          newFontIcon.setIconColor(newValue ? Paint.valueOf(ColorPalette.GREEN.getHexValue())
  ...
  public void login(String username, String password) {
    ...
    MailClient.getInstance().sendCommand(command, response -> {
      if (response.getStatus().equals(ServerResponseStatuses.OK)) {
        ...
        try {
          SystemIOHelper.createUserFolderIfNotExists(resp.getUser().getEmail());
        } catch (IOException e) {
          e.printStackTrace();
        }
        Main.changeScene("client.fxml");
      } else {
        Platform.runLater(() -> new CustomDialog(Main.primaryStage, "error", "Cannot login!",
